...
namespace Mediop.Processors
{
	public class ProcessorArgs : PipelineArgs
	{
		...
		/// <summary>Set by an optimizer when the output format differs from the input (e.g. jpg -> webp).</summary>
		...
		public class ProcessorArgsStatistics
		{
			...
			public float PercentageSaved => SizeBefore == 0 ? 0 : 1 - (SizeAfter / (float)SizeBefore);
